...
//!
//! Every ciphertext is bound to its owning user by passing the `user_id`
//! bytes as AEAD associated data: a ciphertext copied onto another user's
...
impl TokenCrypto {
    ...
    /// Encrypt a token, binding it to `aad` (the owning user's `user_id`
    /// bytes). Returns `nonce || ciphertext+tag`.
    pub fn encrypt(&self, plaintext: &str, aad: &[u8]) -> cja::Result<Vec<u8>> {
    ...
    /// Decrypt `nonce || ciphertext+tag`, validating the tag and that the
    /// ciphertext was produced under the same `aad`.
    pub fn decrypt(&self, data: &[u8], aad: &[u8]) -> cja::Result<String> {
        ...
        let plaintext = self
            ...
            .map_err(|_| {
                eyre!("token decryption failed (tampered data, wrong key, or wrong owner)")
            })?;
...
mod tests {
    ...
    const AAD: &[u8] = b"user-id-aad-0001";
    ...
    fn roundtrip() {
        let c = crypto(0x42);
        let enc = c.encrypt("gho_secret_token_value", AAD).unwrap();
        assert_eq!(c.decrypt(&enc, AAD).unwrap(), "gho_secret_token_value");
    }

    #[test]
    fn tampered_ciphertext_is_rejected() {
        let c = crypto(0x42);
        let mut enc = c.encrypt("gho_secret_token_value", AAD).unwrap();
        // Flip a bit in the ciphertext body (past the nonce).
        let last = enc.len() - 1;
        enc[last] ^= 0x01;
        assert!(c.decrypt(&enc, AAD).is_err());
    }
    ...
    fn wrong_key_is_rejected() {
        let enc = crypto(0x42).encrypt("gho_secret_token_value", AAD).unwrap();
        assert!(crypto(0x43).decrypt(&enc, AAD).is_err());
    }
    ...
    fn wrong_aad_is_rejected() {
        let c = crypto(0x42);
        let enc = c.encrypt("gho_secret_token_value", AAD).unwrap();
        // A ciphertext moved to a different user's row must not decrypt.
        assert!(c.decrypt(&enc, b"user-id-aad-0002").is_err());
        assert!(c.decrypt(&enc, b"").is_err());
    }

    #[test]
    fn nonce_is_unique_across_calls() {
        let c = crypto(0x42);
        let a = c.encrypt("same plaintext", AAD).unwrap();
        let b = c.encrypt("same plaintext", AAD).unwrap();
    ...
    fn truncated_ciphertext_is_rejected() {
        let c = crypto(0x42);
        assert!(c.decrypt(&[0u8; 10], AAD).is_err());
    }
}
